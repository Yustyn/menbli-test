...
class AdminTests(unittest.TestCase):
    ...
    def setUp(self):
        # create SuperAdmin
        self.cmn_admin = Admin(self.VALID_EMAIL, self.VALID_PASSWORD)
        # return super().setUp()
        pass

    def test_create_CmnAdmin(self):
        cmn_admin_val = Admin(self.VALID_EMAIL, self.VALID_PASSWORD)
        self.assertIsInstance(cmn_admin_val, Admin)
        print('Test 1.1: pass.')

    def test_add_admin(self):
        Validate().validate(self.ADMIN_DATA[0], USERS, 'Users')
        response = self.cmn_admin.add_admin(self.ADMIN_DATA)
        self.assertEqual(response, 'Insert done!')
        print('Test 1.3: pass.')
        # self.clear_record('users')

    def test_create_valid_Admin(self):
        cmn_admin_inv = Admin(self.VALID_EMAIL, self.VALID_PASSWORD)
        # self.assertRaises(ValueError, msg='Incorrect email!')
        email_pattern = re.compile(
            r'^([a-zA-Z0-9-_\*\.]+)@([a-zA-Z0-9-]+)(\.[a-zA-Z0-9]+)+$')

        self.assertRegex(self.VALID_EMAIL, email_pattern, 'correct email!')
        print('Test 1.2: pass.')
    pass
